...
function drowPersonages(startIndex = 0, endIndex = 3) {
    cupPreoples = peoples.slice(startIndex, endIndex);

    let HTMLPeoples = cupPreoples.map(el => Personage(el));
...
document.getElementById('next').addEventListener('click', () => {
    let lastInCurrentCup = cupPreoples[cupPreoples.length-1].i;

    if (lastInCurrentCup !== peoples.length-1) {
        drowPersonages(lastInCurrentCup+1, lastInCurrentCup+4);
    } else {
...
document.getElementById('prev').addEventListener('click', () => {
    let firstInCurrentCup = cupPreoples[0].i;

    if (firstInCurrentCup !== 0) {
        drowPersonages(firstInCurrentCup-3, firstInCurrentCup);
    } else {
